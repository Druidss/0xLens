...
### 4.1 基于 shadcn/ui 的组件清单 / shadcn/ui Component List

以下是本产品需要使用的 shadcn/ui 组件及其定制规范：
...
**变体定义 / Variants:**
| Variant | 使用场景 / Use Case | 视觉样式 / Visual Style |
|---|---|---|
| default | 主要操作 | 品牌主色背景 + 白色文字 |
| outline | 次要操作 | 透明背景 + 主色边框 + 主色文字 |
| ghost | 辅助操作 | 透明背景 + 文字色 + hover 时背景 |
| destructive | 危险操作 | 错误色背景 + 白色文字 |

**尺寸定义 / Sizes:**
| Size | 高度 / Height | 内边距 / Padding | 字号 / Font Size |
|---|---|---|---|
| sm | 32px | 12px 16px | text-sm |
| default | 40px | 16px 24px | text-base |
| lg | 48px | 20px 32px | text-lg |

**状态设计 / State Design:**
- **Default:** 默认样式
- **Hover:** 背景色加深 10%, transition 150ms
- **Active/Pressed:** 背景色加深 20%, scale(0.98)
...
- **Focus:** border: 2px solid var(--color-primary-500), outline-ring
...
**自定义样式类 / Custom Classes:**

```css
/* 毛玻璃效果 (Web3风格) / Glassmorphism Effect */
.glass-effect {
...
#### 4.1.4 其他常用组件 / Other Common Components

**需要安装的组件列表 / Components to Install:**
...
每个组件的定制规范参考 shadcn/ui 官方文档，并基于本文档的视觉系统进行调整。

---

### 4.2 自定义业务组件 / Custom Business Components

根据产品功能需求，定义以下自定义组件：

#### 4.2.1 [业务组件1名称] / [Business Component 1]
...
#### 5.1.2 交互元素清单 / Interactive Elements List

##### 元素1: [元素名称] / [Element Name]

**类型 / Type:** [Button / Input / Card / etc.]

**位置 / Position:** [描述在页面中的位置]
...
**交互规范 / Interaction Specs:**
- Hover Delay: [0ms / 100ms]
- Click Debounce: [300ms]
- Animation Duration: [250ms]
- Feedback Type: [Toast / Inline / Modal]

---

#### 5.1.3 微交互设计 / Micro-Interaction Design

##### 微交互1: 按钮点击反馈 / Button Click Feedback

```css
/* 按钮点击动画 / Button Click Animation */
...
.button:active {
  animation: button-click 150ms ease-in-out;
}
```

##### 微交互2: 加载状态 / Loading State
...
#### 5.1.4 表单验证交互 / Form Validation Interaction

**验证时机 / Validation Timing:**
- onBlur: 用户离开输入框时验证
- onChange: 用户输入时实时验证（延迟 300ms）
- onSubmit: 表单提交时完整验证

**错误提示设计 / Error Message Design:**
...
#### 5.1.5 响应式行为 / Responsive Behavior

**桌面端 (≥1024px) / Desktop:**
- [描述桌面端布局和交互]

**平板端 (640px - 1023px) / Tablet:**
- [描述平板端布局和交互]
...
## 6. 设计稿 / Design Mockups

### 6.1 [页面名称] 设计稿 / [Page Name] Mockups

#### 6.1.1 桌面端设计 / Desktop Design

```
┌────────────────────────────────────────────────────────────────┐
│  [Logo]              导航菜单 / Navigation Menu        [User]  │
├────────────────────────────────────────────────────────────────┤
│                                                                │
│  ┌──────────────────────────────────────────────────────────┐ │
│  │                                                          │ │
│  │  [主标题 / Main Title]           [操作按钮 / Action Btn] │ │
│  │                                                          │ │
│  │  [副标题 / Subtitle]                                     │ │
│  │                                                          │ │
│  └──────────────────────────────────────────────────────────┘ │
...
2. **颜色应用 / Color Application:**
   - 背景: var(--bg-primary)
   - 卡片: glass-effect + neon-border
   - 主要内容: var(--bg-secondary)

3. **交互元素 / Interactive Elements:**
   - [操作按钮]: variant="default", size="lg"
   - [卡片]: hover 时轻微上浮 (translateY(-4px))
   - [导航菜单]: 激活项使用 primary 色

---

#### 6.1.2 移动端设计 / Mobile Design
...
### 6.2 组件设计稿 / Component Mockups

#### 6.2.1 [自定义组件1] 设计稿 / [Custom Component 1] Mockup

```
┌─────────────────────────────────────────────────┐
│  [图标]  [标题 / Title]            [操作 / Action] │
├─────────────────────────────────────────────────┤
│                                                 │
│  [主要内容 / Main Content]                       │
│                                                 │
...
## 7. 设计交付清单 / Design Deliverables Checklist

完成设计后，请确保以下内容已交付 / Ensure the following are delivered:

- [ ] ✅ 视觉系统完整定义（色彩、字体、间距、圆角、阴影）
- [ ] ✅ Design Tokens CSS 变量文件
- [ ] ✅ 交互流程图（用户旅程 + 核心功能流程）
- [ ] ✅ shadcn/ui 组件清单和定制规范
- [ ] ✅ 自定义业务组件设计规范
- [ ] ✅ 至少一个完整页面的交互文档（包含所有状态）
- [ ] ✅ 桌面端和移动端设计稿
- [ ] ✅ 响应式断点和适配规则
- [ ] ✅ 微交互设计说明
- [ ] ✅ 无障碍访问 (A11y) 指南
...
## 8. 开发实施建议 / Development Implementation Guide

### 8.1 Tailwind CSS 配置 / Tailwind Config
...
@layer base {
  :root {
    /* 从本文档的视觉系统复制 Design Tokens */
    --color-primary-500: #00f0ff;
    /* ... 其他变量 */
  }
...
### 8.3 shadcn/ui 安装步骤 / shadcn/ui Setup
...
# 3. 根据设计规范修改 components/ui 下的组件样式
```
